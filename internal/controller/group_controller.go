...
func (r *GroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, backend := range groupCR.Spec.Backends {
		...
		if len(usersToAdd) > 0 {
			r.backendLogger.WithField("user_count", len(usersToAdd)).Info("Adding users to Fivetran team")

			err := backendClient.AddUserToTeam(ctx, teamID, usersToAdd)
			if err != nil {
				r.backendLogger.WithError(err).Error("error while adding users to Fivetran team")
		...
		if len(usersToRemove) > 0 {
			r.backendLogger.WithField("user_count", len(usersToRemove)).Info("remopving users from a particvular fivetran team")

			err := backendClient.RemoveUserFromTeam(ctx, teamID, usersToRemove)
			if err != nil {
				r.backendLogger.WithError(err).Error("error while removing users from fivetran treams")
			}
